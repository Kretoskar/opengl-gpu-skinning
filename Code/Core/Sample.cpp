...
void Sample::Initialize()
{
	...
	for (auto& mGPUMeshe : mGPUMeshes)
	{
		mGPUMeshe.UpdateOpenGLBuffers();
	}
	...
	unsigned int numUIClips = static_cast<unsigned int>(mClips.size());
...
void Sample::Update(float deltaTime)
{
	...
	for (unsigned int i = 0, size = static_cast<unsigned int>(mCPUMeshes.size()); i < size; ++i) {
...
void Sample::Render(float inAspectRatio)
{
	...
	Mat4 view = Mat4::LookAt(Vec3(0, 5, 7), Vec3(0, 3, 0), Vec3(0, 1, 0));
	Mat4 model;
