#include "Animation/Public/TransformTrack.h"
...
float TransformTrack::GetStartTime()
{
    ...
    if (mRotation.Size() > 1)
    {
        const float rotationStart = mRotation.GetStartTime();
    ...
    if (mScale.Size() > 1)
    {
        const float scaleStart = mScale.GetStartTime();
...
float TransformTrack::GetEndTime()
{
    ...
    if (mRotation.Size() > 1)
    {
        const float rotationEnd = mRotation.GetEndTime();
    ...
    if (mScale.Size() > 1)
    {
        const float scaleEnd = mScale.GetEndTime();
